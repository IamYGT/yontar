						...
						<div class="table-responsive"> 
							<div id="example5_wrapper" class="dataTables_wrapper no-footer">
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									<thead>
										<tr role="row"> 
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 25%">Domain</th>  
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 25%">Bitiş Tarihi</th>  
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 25%">Güncellenme Tarihi</th>  
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 25%">Eklenme Tarihi</th>  
									...
									<tbody> 
<?php
									$list = $db->query("SELECT * FROM ".TABLE." ORDER BY id DESC LIMIT $baslangic,$limit"); 
										if ($list->rowCount()){
											foreach($list as $row){ 
?>
										<tr role="row" class="even"> 
											<td class=""><?php echo $row["strDomain"]; ?></td>
											<td class=""><?php echo $row["expiryTime"]; ?></td>
											<td class=""><?php echo $row["utime"]; ?></td>
											<td class=""><?php echo $row["time"]; ?></td>
											<td class=""><?php echo ($row["iStatus"] == 1 ? '<span class="badge badge-rounded badge-secondary">Aktif</span>' : '<span class="badge badge-rounded badge-danger">Pasif</span>'); ?></td>
											<td class="sorting_1">
												<div class="d-flex">
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=edit&id=<?php echo $row["id"]; ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>
													<a class="contact-icon" href="<?php echo AREA; ?>?do=delete&id=<?php echo $row["id"]; ?>"><i class="fas fa-trash-alt"></i></a>
												</div>
											</td>
										</tr> 
<?php 	
...
<?php 
				...
				if(isset($submitControl)){
					if(!empty($_POST['strDomain'])) {
						$insert = $db->prepare("INSERT INTO ".TABLE." SET strDomain = ?, expiryTime = ?, iStatus = ?");
						$insert->execute(array($strDomain, $expiryTime, $iStatus));
					...
					<div class="col-xl-8 col-xxl-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Lisans Ekle</h4>
                            </div>
                            <div class="card-body">   
								<div class="row">
									<div class="col-lg-12 mb-2">
										<div class="form-group">
											<label class="text-label">Domain</label>
											<input type="text" name="strDomain" class="form-control" placeholder="Domain" required="">
										</div> 
									</div>
								</div>
...
<?php 
				...
				if ($row_check->rowCount() > 0) {
					if(isset($submitControl)){
						if(!empty($_POST['strDomain'])) {   
							$update = $db->prepare("UPDATE ".TABLE." SET strDomain = ?, expiryTime = ?, iStatus = ? WHERE id = ?");
							$update->execute(array($strDomain, $expiryTime, $iStatus, $id));						
							if ($update) {
...
?>
			...
			<form action="#" method="POST" enctype="multipart/form-data" id="form" style="display: contents; "> 
					<div class="col-xl-8 col-xxl-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Lisans Düzenle</h4>
                            </div>
                            <div class="card-body">   
								<div class="row">
									<div class="col-lg-12 mb-2">
										<div class="form-group">
											<label class="text-label">Domain</label>
											<input type="text" name="strDomain" class="form-control" placeholder="Domain" required="" value="<?php echo $row_info["strDomain"]; ?>">
										</div> 
									</div>
								</div>
								<div class="step-footer pull-right">
					...
					<div class="col-xl-4 col-xxl-4">
                        <div class="card">
                            <div class="card-body p-3">   
								<div class="form-group"> 
									<label for="">Bitiş Tarihi</label>
									 <input type="text" class="form-control" name="expiryTime" value="<?php echo $row_info["expiryTime"]; ?>" />
								</div> 	
								<div class="form-group"> 
									<label for="">Yayın Durumu</label>
									<select name="iStatus" class="form-control">
...
<?php 
				...
				$row_check->execute(array($id)); 
				if ($row_check->rowCount() > 0) {
					$update = $db->exec("DELETE FROM ".TABLE." WHERE id = {$id};");
					if($update) {
