							...
							<div id="example5_wrapper" class="dataTables_wrapper no-footer">
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									<thead>
										...
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 80%;">Başlık</th>
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Postition: activate to sort column ascending" style="width: 23%;">Tarih</th>
...
<?php
									$list = $db->query("SELECT * FROM ".TABLE." WHERE dil_id = '".LANGUAGE_DEFAULT."' ORDER BY row ASC LIMIT $baslangic,$limit"); 
										if ($list->rowCount()){
											foreach($list as $row){ 
?>
										<tr id="item-<?php echo $row["menu_ust_id"]; ?>" role="row" class="even"> 
											<td class="sortable ui-sortable-handle" style="width:20px"><i class="fa fa-sort"></i></td>
											<td class=""><?php echo $row["menu_baslik"]; ?></td>
											<td class=""><?php echo $row["menu_tarih"]; ?></td>
											<td class="sorting_1">
												<div class="d-flex">
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=edit&id=<?php echo $row["menu_ust_id"]; ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>
													<a class="contact-icon" href="<?php echo AREA; ?>?do=delete&id=<?php echo $row["menu_ust_id"]; ?>"><i class="fas fa-trash-alt"></i></a>
												</div>
											</td>
										</tr> 
<?php 	
...
<?php 
				...
				if(isset($submitControl)){
					if(!empty($_POST['menu_baslik']) || !empty($_POST['menu_aciklama'])) {
						$LastID = $db->query("SELECT menu_id FROM ".TABLE." ORDER BY menu_id DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
						$LastID = $LastID["menu_id"]+1;
						$upload = new Upload($_FILES['menu_resim']);
						if ($upload->uploaded) { 
								$upload->file_auto_rename = true;
								$upload->process("../uploads/categories");
							if ($upload->processed){  
								$menu_resim = $upload->file_dst_name;
							} else { 
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET menu_baslik = ?, menu_seo = ?, menu_aciklama = ?, menu_resim = ?, dil_id = ?, menu_durum = ?, menu_ust_id = ?");
								$insert->execute(array($menu_baslik[$key], Seo_Link_Cevir($menu_baslik[$key]), $menu_aciklama[$key], $menu_resim, $key, $menu_durum, $LastID));
					...
					<div class="col-xl-8 col-xxl-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Menü Ekle</h4>
                            </div>
                            <div class="card-body"> 
													...
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																...
																<input type="text" name="menu_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Başlık" required="">
															</div>
														</div>  
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Açıklama</label>
																<textarea class="form-control ckeditor" name="menu_aciklama[<?php echo $LANGUAGE_CODE; ?>]"></textarea>
															</div>
														</div>
													</div>
									...
									<div class="fileinput fileinput-new" data-provides="fileinput">
										...
										<div>
											<span class="btn btn-primary btn-sm btn-file">
												...
												<input type="file" name="menu_resim" accept="image/*" id="menu_resim">
											</span>
...
<?php 
				...
				$row_check = $db->prepare("SELECT * FROM ".TABLE." WHERE menu_ust_id = ?");
				$row_check->execute(array($id)); 
				if ($row_check->rowCount() > 0) {
					if(isset($submitControl)){
						if(!empty($_POST['menu_baslik']) || !empty($_POST['menu_aciklama'])) {
							$image_info = $db->query("SELECT * FROM ".TABLE." WHERE menu_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
								$upload = new Upload($_FILES['menu_resim']);  
								if(!isset($_FILES['menu_resim']['name'])) {
									$menu_resim = null;
								} else if(!empty($_FILES['menu_resim']['name'])) {
									$upload->file_auto_rename = true; 
									$upload->process("../uploads/categories");
									if ($upload->processed) {
										$menu_resim = $upload->file_dst_name;
									...
									$menu_resim = $image_info["menu_resim"];
								} 
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									$key = str_replace("form_","",$k);
									$update = $db->prepare("UPDATE ".TABLE." SET  menu_baslik = ?, menu_seo = ?, menu_aciklama = ?, menu_resim = ?, menu_durum = ? WHERE menu_ust_id = ? AND dil_id = ?");
									$update->execute(array($menu_baslik[$key], Seo_Link_Cevir($menu_baslik[$key]), $menu_aciklama[$key], $menu_resim, $menu_durum, $id, $key));
								}
							}
							if ($update) {
					...
					$row_info = $db->query("SELECT * FROM ".TABLE." WHERE menu_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
?>
				...
				<form action="#" method="POST" enctype="multipart/form-data" id="form" style="display: inline-flex; "> 
					<div class="col-xl-8 col-xxl-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Menü Düzenle</h4>
                            </div>
                            <div class="card-body"> 
								<div class="step-app" id="demo">
									<ul class="step-steps">
												...
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Başlık</label>
																<input type="text" name="menu_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Başlık" required="" value="<?php echo GetTableValue("menu_baslik",TABLE,"where menu_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>  
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Açıklama</label>
																<textarea class="form-control ckeditor" name="menu_aciklama[<?php echo $LANGUAGE_CODE; ?>]"><?php echo GetTableValue("menu_aciklama",TABLE,"where menu_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?></textarea>
															</div>
														</div>
													</div>
													<input type="hidden" name="form_<?php echo $LANGUAGE_CODE; ?>" value="<?php echo $LANGUAGE_CODE; ?>" />
					...
					<div class="col-xl-4 col-xxl-4">
                        <div class="card">
                            <div class="card-body p-3">  
								...
								<label>Resim</label>
								<small class="form-text text-muted">Seçmiş olduğunuz resim veri içeriğinde kullanılmaktadır.</small> 
								<?php if(empty($row_info["menu_resim"])) {?>
										<div class="fileinput fileinput-new" data-provides="fileinput">
											<div class="fileinput-preview thumbnail" data-trigger="fileinput" style="width: 100%;height: 190px;line-height: 190px;">
								...
								<?php }else{?> 
										<div class="fileinput fileinput-exists" data-provides="fileinput">
											...
											<div class="fileinput-preview fileinput-exists thumbnail" style="max-width: 303px; max-height: 190px; line-height: 190px;"><img src="../uploads/categories/<?php echo $row_info["menu_resim"]; ?>" class="img-fluid img-thumbnail"></div>
											<div>
												<span class="btn btn-primary btn-sm btn-file">
													<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
													<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
													<input type="file" name="menu_resim" id="menu_resim" >
								...
								<div class="form-group"> 
									<label for="">Yayın Durumu</label>
									<select name="menu_durum" class="form-control">
										<option value="1" <?php if ($row_info["menu_durum"] == "1") { echo "selected";}?>>Aktif</option>
										<option value="2" <?php if ($row_info["menu_durum"] == "2") { echo "selected";}?>>Pasif</option> 
									</select>
								</div> 
                            </div>
                        </div>
...
<?php 
				...
				$row_check->execute(array($id)); 
				if ($row_check->rowCount() > 0) {
					$update = $db->exec("DELETE FROM ".TABLE." WHERE menu_ust_id = {$id};");
					if($update) {
		...
		$( "#sortable" ).sortable({
			...
			stop: function (event, ui) {
				...
				$.ajax({
					...
					dataType: "json",
					data: data,
					url: "/include/ajax.php?do=menu&action=row",
